...
def create_db(host_address, port_address, username, user_password):
    ...
    con = mysql.connector.connect(host=host_address, port=port_address, database='chatbot', user=username, password=user_password)
    cur = con.cursor(dictionary=True)
    cur.execute("""CREATE TABLE IF NOT EXISTS chatbot_answers(
    id INT NOT NULL AUTO_INCREMENT,
    text_time DATETIME NULL, 
    user_text VARCHAR(4) NULL,
    bot_answer VARCHAR(100) NULL,
    PRIMARY KEY (id),
...
def insert_into_db(text_time, user_text, bot_answer, host_address, port_address, username, user_password):
    con = mysql.connector.connect(host=host_address, port=port_address, database='chatbot', user=username, password=user_password)
    cur = con.cursor(dictionary=True)
    insert_line = f"INSERT INTO chatbot_answers (text_time, user_text, bot_answer) VALUES ('{text_time}', '{user_text}', '{bot_answer}')"
